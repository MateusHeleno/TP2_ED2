...
bool lerRegistroNaPosicao(FILE *arq, Registro *reg, int pos, Metricas *metricas) {
    if (fseek(arq, (long)pos * TAM_REGISTRO_TXT, SEEK_SET) != 0) return false;
...
void quicksortExternoRec(FILE *arq, int esq, int dir, Metricas *metricas) {
    ...
    if (dir - esq + 1 <= TAM_MEMORIA) {
        ...
        //Ordena usando o quicksort da stdlib
        qsort(area, qtd, sizeof(Registro), compararRegistros);
    ...
    qsort(area, TAM_MEMORIA, sizeof(Registro), compararRegistros);
